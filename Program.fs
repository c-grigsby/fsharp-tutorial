...
// Pure Functions
// Not Pure (Causes a side effect)
let addOneToValue x = 
    printfn $"Hi I am a side effect!\nThis function is now adding x + 1, where x = %d{x}.\n"
    x + 1
// Pure 
let addOneToValue_Pure x = x + 1
...
let main argv =
    ...
    printfn "The following function is not pure, just watch...\n" 
    let newValue = addOneToValue 4
    printfn "New Value = %d" newValue
    printfn "The next function IS a pure function..."
    let nextNewValue = addOneToValue_Pure 4
    printfn "New Value = %d" nextNewValue
